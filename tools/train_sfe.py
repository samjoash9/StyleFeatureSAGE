"""
Training script for AGE with FSE Inverter
"""
import os
import sys
import pprint
...
from models.age_with_fse import AGEWithFSE  # Our modified model
...
def train():
    ...
    # Initialize network with FSE
    net = AGEWithFSE(opts).to(device)
    
    # Setup parameters for optimization
    params = list(net.ax.parameters())
    
    # Add feature editor parameters if training them
    if hasattr(opts, 'train_feature_editor') and opts.train_feature_editor:
        if hasattr(net.encoder, 'encoder'):  # FSEFull
            params += list(net.encoder.encoder.parameters())
        else:  # FSEInverter - train the fuser
            for name, param in net.encoder.named_parameters():
                if 'fuser' in name:
                    params.append(param)
        
        if dist.get_rank() == 0:
            print("Training feature editor along with Ax module")
    ...
    if opts.sparse_lambda > 0:
        sparse_loss_fn = sparse_loss.SparseLoss().to(device).eval()
    else:
        sparse_loss_fn = None
        
    if opts.orthogonal_lambda > 0:
        orthogonal_loss_fn = orthogonal_loss.OrthogonalLoss(opts).to(device).eval()
    else:
        orthogonal_loss_fn = None

    global_step = 0
    net.train()
    
    # Feature scale scheduling
    if hasattr(opts, 'feature_scale_schedule') and opts.feature_scale_schedule:
        feature_scale = 0.0  # Start with no feature editing
    else:
        feature_scale = getattr(opts, 'feature_scale', 1.0)

    while global_step < opts.max_steps:
        # Update feature scale if using schedule
        if hasattr(opts, 'feature_scale_schedule') and opts.feature_scale_schedule:
            feature_scale = min(1.0, global_step / opts.feature_scale_warmup)
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            # Forward pass with feature editing
            outputs = net.forward(
                x, 
                av_codes, 
                return_latents=True,
            ...
            loss, loss_dict, id_logs = calc_loss(
                opts, outputs, y, orthogonal_loss_fn, sparse_loss_fn, lpips
            )
            loss.backward()
            optimizer.step()
            ...
            if dist.get_rank() == 0:
                if global_step % opts.image_interval == 0 or (global_step < 1000 and global_step % 25 == 0):
                    parse_and_log_images(
                        opts, logger, global_step, id_logs, x, y, 
                        outputs['y_hat'], title='images/train/faces'
                    )
                if global_step % opts.board_interval == 0:
                    ...
                    # Log feature scale if using schedule
                    if hasattr(opts, 'feature_scale_schedule') and opts.feature_scale_schedule:
                        logger.add_scalar('train/feature_scale', feature_scale, global_step)
            ...
            if global_step % opts.val_interval == 0 or global_step == opts.max_steps:
                if dist.get_rank() == 0:
                    val_loss_dict = validate(
                        opts, net, orthogonal_loss_fn, sparse_loss_fn, lpips, 
                        valid_dataloader, device, global_step, logger, feature_scale
                    )
                    ...
                    val_loss_dict = validate(
                        opts, net, orthogonal_loss_fn, sparse_loss_fn, lpips,
                        valid_dataloader, device, global_step, feature_scale=feature_scale
                    )
...
def validate(opts, net, orthogonal, sparse, lpips, valid_dataloader, device, 
             global_step, logger=None, feature_scale=1.0):
    net.eval()
    agg_loss_dict = []
    for batch_idx, batch in enumerate(valid_dataloader):
        ...
        with torch.no_grad():
            x, y, av_codes = x.to(device).float(), y.to(device).float(), av_codes.to(device).float()
            outputs = net.forward(
                x, av_codes, return_latents=True, feature_scale=feature_scale
            )
        ...
        if dist.get_rank() == 0:
            parse_and_log_images(
                opts, logger, global_step, id_logs, x, y, outputs['y_hat'], 
                title='images/valid/faces', subscript='{:04d}'.format(batch_idx)
            )
